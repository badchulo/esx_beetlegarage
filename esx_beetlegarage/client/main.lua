    ...
    ESX.TriggerServerCallback('beetle_garage:getVehicleList', function(cb)
        if cb ~= "notCars" and not IsPedInAnyVehicle(PlayerPedId()) and not inGarage then
            ...
            for _, v in pairs(cb) do
                if v.stored == 1 and v.shared == false then
                    table.insert(vehicles, {
                        label = _U("SHOW_VEHICLE_STORED", GetLabelText(GetDisplayNameFromVehicleModel(v.vehicle.model))),
                        value = v.vehicle.plate
                    })
				elseif v.stored == 1 and v.shared == true then
					table.insert(vehicles, {
							label = _U("SHOW_VEHICLE_SHARED", GetLabelText(GetDisplayNameFromVehicleModel(v.vehicle.model))),
							value = v.vehicle.plate
						})
				
                else
                    if Config.DisableImpound and v.shared == false then
                        table.insert(vehicles, {
                            label = _U("SHOW_VEHICLE_NOT_STORED", GetLabelText(GetDisplayNameFromVehicleModel(v.vehicle.model))),
                            value = v.vehicle.plate
                        })
                    elseif v.shared == false then
                        table.insert(vehicles, {
