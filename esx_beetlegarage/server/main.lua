...
ESX.RegisterServerCallback("beetle_garage:validCar", function(source, cb)
    ...
    MySQL.Async.fetchScalar('SELECT COUNT(owner) FROM owned_vehicles WHERE (owner=@identifier OR segundallave=@identifier) AND plate=@plate', {
...
ESX.RegisterServerCallback("beetle_garage:saveVehicle", function(source, cb, vehprop)
    ...
    MySQL.Async.execute("UPDATE owned_vehicles SET vehicle=@vehdata, owned_vehicles.stored=1 WHERE (owner=@identifier OR segundallave=@identifier) AND plate=@plate", {
        ['@vehdata'] = vehJson,
        ['@identifier'] = xPlayerIdentifier,
        ['@plate'] = plate
    }, function(result)
...
ESX.RegisterServerCallback("beetle_garage:getVehicleList", function(source, cb)
    ...
    MySQL.Async.fetchAll("SELECT owner, vehicle, segundallave, owned_vehicles.stored FROM owned_vehicles WHERE (owner=@identifier OR segundallave=@identifier)", {
        ['@identifier'] = xPlayerIdentifier
    }, function(result)
        for _, v in pairs(result) do
			local shared = false
            local vehicle = json.decode(v.vehicle)
			if (xPlayerIdentifier == v.segundallave) and ( xPlayerIdentifier ~= v.owner  ) then
				shared = true
			end
...
ESX.RegisterServerCallback("beetle_garage:changeStatus", function(source, cb, plate)
    ...
    MySQL.Async.execute("UPDATE owned_vehicles SET owned_vehicles.stored=0 WHERE plate=@plate AND (owner=@identifier OR segundallave=@identifier)", {
        ["@plate"] = plate,
        ["@identifier"] = xPlayerIdentifier
    }, function(result)
        if result then
